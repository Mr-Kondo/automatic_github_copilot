...
さらに、既定では `log/` 配下の最新ログファイルを毎イテレーションで探索し、実装プロンプト向けに末尾を抽出して `.copilot-loop-logs/iter-*/latest_app_log_snippet.txt` に保存します。実装エージェントにはこの抽出ファイルのパスが追加コンテキストとして渡されます。

実装・レビュー結果のサマリは管理用 Markdown ログ `.copilot-loop-logs/context.md` に追記されます。このファイルが `--log-context-max-bytes` を超えると、世代管理でローテーションされます（`.1`, `.2`, ...）。保持世代数は `--log-context-keep` で制御できます。

`--no-log-context` を指定すると、上記のログ注入・管理ログ追記は無効化され、従来どおりの動作になります。
